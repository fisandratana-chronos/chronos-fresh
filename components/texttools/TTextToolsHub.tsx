...
function TTextToolsHub({ onBack }: { onBack?: () => void }) {
  ...
  // Image / Converters Hub) — tsy miasa raha efa eo anaty input/textarea/
  ...
  // Mi-filtrer ireo TEXT_CATEGORIES/tools araka ny toolSearch (anarana sy
  // description, amin'ny lang eo am-pihodinana). Foana ny search → daholo.
  const filteredCategories = React.useMemo(() => {
    const q = toolSearch.trim().toLowerCase()
    if (!q) return TEXT_CATEGORIES
  ...
  const filteredToolsFlat = React.useMemo(
    () => filteredCategories.flatMap(cat => cat.tools.map(id => TEXT_TABS.find(t => t.id === id)).filter(Boolean) as typeof TEXT_TABS),
    [filteredCategories]
  )
      ...
      <div className="texttools-layout" style={{ display: 'flex', flex: 1, minHeight: 0, width: '100%', position: 'relative' }}>
        ...
        <div style={{ width: '100%' }}>
          ...
          <div style={{ position: 'relative', marginBottom: 16 }}>
            ...
            <input
              ref={toolSearchRef}
              id="text-tools-search"
              name="text-tools-search"
              type="text"
              autoComplete="off"
              value={toolSearch}
