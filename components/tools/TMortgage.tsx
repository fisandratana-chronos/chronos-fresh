  ...
  const Inp = ({ label, value, onChange, placeholder, unit }: { label: string, value: string, onChange: (v: string) => void, placeholder: string, unit?: string }) => (
    <div style={{ flex: 1 }}>
      <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: T.muted, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        {label} {unit && <span style={{ color: T.muted2 }}>({unit})</span>}
      </label>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {result ? (
        <div style={{ background: `${blue}10`, border: `2px solid ${blue}`, borderRadius: 14, padding: 24, textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: T.muted, marginBottom: 4 }}>{lang === 'fr' ? 'Mensualité' : 'Monthly Payment'}</div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <div style={{ background: T.chipBg, color: T.text, borderRadius: 10, padding: '10px 20px', fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
